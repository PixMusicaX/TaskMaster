...
export default function MoodPicker({ mood, content, onMoodChange, onContentChange, onSave }: MoodPickerProps) {
  const moods = [
        ...
        {moods.map((m) => (
          <button
            ...
            className={cn(
              ...
              m.bg,
              mood === m.value ? "bg-white/10 border-white/20 scale-110 shadow-lg" : "opacity-50 hover:opacity-100"
            )}
          >
